...
| 角色 | 背景 | 特征 |
|------|------|------|
| 用户消息 | User Message Card 色（Primary 10%） | 文本 + 附件摘要；长按卡片弹出操作菜单 |
| AI 消息 | Card 色（Black/White 5%） | "AI" 标签（Primary 色）+ Markdown 渲染；长按卡片弹出操作菜单 |
| 工具结果 | Card Nested 色 | 可折叠，12dp 圆角，等宽字体显示参数和结果 |

用户消息带有主题色调，AI 消息为中性灰底，视觉上自然区分角色。消息操作统一通过长按卡片弹出菜单触发，气泡内不放置常驻操作按钮；长消息收起时以末尾省略号提示。长按卡片任意位置（含 Markdown 文本区）均可触发，并伴随覆盖整卡片的圆角波纹反馈；波纹与菜单仅在长按落点未被上层交互元素（链接、代码复制按钮、工具/思考块、可选文本等）消费时显示，被消费区域交由其自身处理（如链接点击、文本选择）。
...
> 消息的复制 / 编辑 / 删除 / 重试不再使用常驻图标按钮，统一收进长按弹出菜单。
...
- 菜单项带图标（次要文字色着色，`setForceShowIcon`）：复制 / 编辑 / 删除；AI 末条及其对应用户消息附加重试，用户长消息附加展开/收起
- 通过 `android:popupMenuStyle` 全局统一圆角，附件等其他菜单一致

### 回到底部按钮

- 列表未滚动到底（下方仍有内容）时，在输入栏上方**水平居中**淡入的毛玻璃圆形按钮（44dp，向下图标）
- 毛玻璃背景 + 阴影 + Primary 图标，无边框；与输入栏一致的模糊与圆角裁剪
- AI 输出过程中，按钮边框显示环形进度（`CircularProgressIndicator` 不确定态），提示仍在生成
- 点击平滑滚动（动画已加快）到最新消息并恢复跟随输出；Shell 确认条显示时隐藏
- 流式输出期间的自动吸附为**无动画实时跟随**（仅按钮点击与显隐使用动画）
