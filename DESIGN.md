...
用户消息带有主题色调，AI 消息为中性灰底，视觉上自然区分角色。消息操作统一通过长按卡片弹出菜单触发，气泡内不放置常驻操作按钮；长消息收起时以末尾省略号提示。
...
- 长按消息卡片触发；圆角 16dp，不透明悬浮底色（`mote_popup_background`，约等于 Card 叠加在背景上的不透明等价色），低投影（3dp）
- 菜单项：复制 / 编辑 / 删除；AI 末条附加重试，用户长消息附加展开/收起
- 通过 `android:popupMenuStyle` 全局统一圆角，附件等其他菜单一致

### 回到底部按钮

- 列表未滚动到底（下方仍有内容）时，在输入栏上方淡入的 mini FAB（向下图标）
- 不透明悬浮底色（`mote_popup_background`）+ Primary 图标，无边框，低投影
- 点击平滑滚动到最新消息并恢复跟随输出；Shell 确认条显示时隐藏
