...
用户消息带有主题色调，AI 消息为中性灰底，视觉上自然区分角色。消息操作统一通过长按卡片弹出菜单触发，气泡内不放置常驻操作按钮；长消息收起时以末尾省略号提示。长按卡片任意位置（含 Markdown 文本区）均可触发，并伴随覆盖整卡片的圆角波纹反馈；思考过程 / 工具结果等可选文本区域长按仍为文本选择，不弹菜单。
...
- 菜单项：复制 / 编辑 / 删除；AI 末条附加重试，用户长消息附加展开/收起
- 通过 `android:popupMenuStyle` 全局统一圆角，附件等其他菜单一致

### 回到底部按钮

- 列表未滚动到底（下方仍有内容）时，在输入栏上方**水平居中**淡入的小号毛玻璃圆形按钮（36dp，向下图标）
